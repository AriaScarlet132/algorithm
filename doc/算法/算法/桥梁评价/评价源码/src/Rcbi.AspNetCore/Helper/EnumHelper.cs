...
namespace Rcbi.AspNetCore.Helper
{
    public sealed class EnumHelper
    {
        ...
        public static object GetEnum<T>(string equalsValue)
        {
            foreach (var item in Enum.GetValues(typeof(T)))
            {
                if (equalsValue.ToLower() == item.ToString().ToLower())
                    return item;
            }
            return null;
        }
        ...
        public static IList<KeyValuePair<string, string>> GetDictionary(Type type)
        {
            if (!type.IsEnum)
            {
                throw new InvalidOperationException("Not Found enum type : " + type.FullName);
            }
        ...
        public static string GetCachedDescriptionByValue(Type type, string val)
        {
            IList<KeyValuePair<string, string>> dictionary = GetCachedDictionary(type);
            foreach (KeyValuePair<string, string> keyValuePair in dictionary)
            {
                if (keyValuePair.Key == val)
                {
        ...
        /// <returns></returns>
        public static string GetCachedDescriptionByValue(Type type, int? val)
        {
            if (!val.HasValue)
            {
                return string.Empty;
            ...
            foreach (KeyValuePair<string, string> keyValuePair in dictionary)
            {
                if (keyValuePair.Key == val.ToString())
                {
                    return keyValuePair.Value;
                }
            }
