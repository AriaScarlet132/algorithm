...
def cal(TaskNO, DataSource_StartDate, DataSource_EndDate, ProjectID, conn, cursor, engine):
    ...
    def get_weight_from_db(parent_code, code_list):  # 从权重表格中提取权重数据
        sql4 = 'select * from ' + 'tb_bridge_assessment_weights' + ' where code=\'' + code_list + '\' and project_code=\'CSYQ\''
    ...
    def facility_disease(code):  # 根据code提取构件病害列表
        sql = 'select * from tb_bridge_yq_cs_disease where facility_code =\'' + code + '\''
    ...
    def facility_get_DP(facility_code, disease_name, disease_grade):  # 提取单个病害的扣分值
        sql = 'select * from tb_bridge_yq_cs_disease where facility_code =\'' + facility_code + '\' and disease_name=\'' + disease_name + '\''
        df_disease = pd.read_sql(sql=sql, con=engine)
        col = df_disease.columns
        DP = 0
        for i in range(len(col)):
            if disease_grade == df_disease[col[i]].values[0]:
                DP = df_disease[col[i + 1]].values[0]
        return DP
        ...
        for i in newdf.index:  # 遍历每条病害记录
            ...
            for j in range(len(disease_list)):  # 对于构件对应的disease_list，遍历病害，添加等级
                try:
                    ...
                    continue
            points = method.csyq_dp_points(DP)  # 计算单个构件得分值
    ...
    def facility_bujian_values(outputtable):  # 计算部件得分——构件得分平均值
    ...
    def get_output_list():  # 获得各层次输出得分数组
        ...
        # 上部结构下属
        zl = Output('SBJG', '总体技术状况得分-上部结构', 'ZL', '主梁')
        ...
        delete_flag5 = facility('SSBGZL', 'tb_bridge_bgzk_zlxt_zl_checkvalue', 'tb_bridge_bgzk_zlxt_zl_output')  # 叠合梁主梁
        zl.delete_flag = min(delete_flag5, delete_flag4, delete_flag3, delete_flag2, delete_flag1)
        zl.points = facility_bujian_values('tb_bridge_bgzk_zlxt_zl_output')
        zl.get_grade()

        hxlx = Output('SBJG', '总体技术状况得分-上部结构', 'HXLX', '横向联系')
        delete_flag1 = facility('SSBJFF', 'tb_bridge_bgzk_zzxt_znq_checkvalue', 'tb_bridge_bgzk_zzxt_znq_output')  # 铰缝
        delete_flag2 = facility('SSBHGB', 'tb_bridge_bgzk_zzxt_znq_checkvalue',
                                'tb_bridge_bgzk_zzxt_znq_output')  # 横隔板
        delete_flag3 = facility('SSBSJF', 'tb_bridge_bgzk_zzxt_znq_checkvalue',
                                'tb_bridge_bgzk_zzxt_znq_output')  # 湿接缝
        delete_flag4 = facility('SSBHLL', 'tb_bridge_bgzk_zzxt_znq_checkvalue',
                                'tb_bridge_bgzk_zzxt_znq_output')  # 横梁（含连接件）

        hxlx.delete_flag = min(delete_flag4, delete_flag3, delete_flag2, delete_flag1)
        hxlx.points = facility_bujian_values('tb_bridge_bgzk_zzxt_znq_output')
        hxlx.get_grade()

        weight_re_average([zl, hxlx])
        ...
        qtps = Output('QMX', '总体技术状况得分-桥面系', 'QTPS', '桥头平顺')
        qtps.delete_flag = facility('SQMQTB', 'tb_bridge_bgzk_qmx_sszz_checkvalue',
                                    'tb_bridge_bgzk_qmx_sszz_output')  # 桥头平顺
        sql = 'select points from tb_bridge_bgzk_qmx_sszz_output where task_no="' + task_no + '" and project_code="' + project_code + '" and facility_code like \'%%SQMQTB%%\''
        df = pd.read_sql(sql=sql, con=engine)
        if df.empty:
            a = 1
        else:
            points = df['points'].values
            qtps.points = np.average(points)
        qtps.get_grade()

        weight_re_average([qmpz, sszz, rxd, qtps])

        # 下部结构下属
        zz = Output('XBJG', '总体技术状况得分-下部结构', 'ZZ', '支座')
        delete_flag1 = facility('SXBBXZ', 'tb_bridge_bgzk_zzxt_zz_checkvalue',
                                'tb_bridge_bgzk_zzxt_zz_output')  # 板式橡胶支座
        delete_flag2 = facility('SXBPXZ', 'tb_bridge_bgzk_zzxt_zz_checkvalue',
                                'tb_bridge_bgzk_zzxt_zz_output')  # 盆式橡胶支座
        delete_flag3 = facility('SXBGZZ', 'tb_bridge_bgzk_zzxt_zz_checkvalue', 'tb_bridge_bgzk_zzxt_zz_output')  # 钢支座
        delete_flag4 = facility('SXBHBZ', 'tb_bridge_bgzk_zzxt_zz_checkvalue',
        ...
        ds = Output('XBJG', '总体技术状况得分-下部结构', 'DS', '墩身')
        ds.delete_flag = facility('SXBQDS', 'tb_bridge_bgzk_xbjg_qd_checkvalue',
                                  'tb_bridge_bgzk_xbjg_qd_output')  # 桥墩墩身
        sql = 'select points from tb_bridge_bgzk_xbjg_qd_output where task_no="' + task_no + '" and project_code="' + project_code + '" and facility_code like \'%%SXBQDS%%\''
        df = pd.read_sql(sql=sql, con=engine)
        if df.empty:
            a = 1
        else:
            points = df['points'].values
            ds.points = np.average(points)
        ds.get_grade()

        gl = Output('XBJG', '总体技术状况得分-下部结构', 'GL', '盖梁')
        gl.delete_flag = facility('SXBQGL', 'tb_bridge_bgzk_xbjg_qd_checkvalue',
                                  'tb_bridge_bgzk_xbjg_qd_output')  # 盖梁系梁
        sql = 'select points from tb_bridge_bgzk_xbjg_qd_output where task_no="' + task_no + '" and project_code="' + project_code + '" and facility_code like \'%%SXBQGL%%\''
        df = pd.read_sql(sql=sql, con=engine)
        if df.empty:
            a = 1
        else:
            points = df['points'].values
            gl.points = np.average(points)
        gl.get_grade()

        jc = Output('XBJG', '总体技术状况得分-下部结构', 'JC', '基础')
        jc.delete_flag = facility('SXBJCC', 'tb_bridge_bgzk_xbjg_jcct_checkvalue', 'tb_bridge_bgzk_xbjg_jcct_output')
        jc.points = facility_bujian_values('tb_bridge_bgzk_xbjg_jcct_output')
        jc.get_grade()
        weight_re_average([gl, ds, jc, zz])
        ...
        sbjg.points = method.weighted_avg([zl, hxlx])  # 根据规范，加权平均计算上部结构得分
        sbjg.get_grade()
        xbjg.points = method.weighted_avg([gl, ds, jc, zz])
        xbjg.get_grade()
        qmx.points = method.weighted_avg([qmpz, sszz, rxd, qtps])
        qmx.get_grade()

        total.points = method.weighted_avg([sbjg, xbjg, qmx])
        total.get_grade()

        output = {'total': total, 'sbjg': sbjg, 'xbjg': xbjg, 'qmx': qmx, 'qmpz': qmpz, 'qtps': qtps, 'sszz': sszz,
                  'rxd': rxd, 'zl': zl, 'hxlx': hxlx, 'gl': gl, 'ds': ds, 'jc': jc, 'zz': zz}
