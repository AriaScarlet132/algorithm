...
import calculate_structure_CSYQ  # 公路引桥的计算函数，对应msg4
import send
import os
import json
import traceback
...
def msg4(body, msgdate):  # 主桥结构的计算函数
    ...
    try:
        ...
        cursor = conn.cursor()
        calculate_structure_CSYQ.cal(body['TaskNO'], body['DataSource_StartDate'], body['DataSource_EndDate'],
...
def callback(channel, method, properity, body):
    ...
    try:
        if body_json['Facility_Type'] == 'Bridge_CSYQ':  # 如果是引桥的评价
...
exchange = 'MQ_Bridge_CSYQ_TJEVA'
queue = 'MQ_Bridge_CSYQ_TJEVA'
username = 'rabbit_tbmp'
pwd = 'rabbit_tbmp'
user_pwd = pika.PlainCredentials(username, pwd)
while True:
