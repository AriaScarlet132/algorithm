...
def cal_USI(xtidata,ttidata,iaidata,iridata,ecidata,rating,days,weight):
    ...
    if xtidata.empty:
       ...
       ### 看一下时间
        ...
        xtiData['time1']=(xtiData['complaint_finish_time'] - xtiData['complaint_acceptance_time']).dt.seconds/3600
        a = xtiData.groupby(['monitor_year', 'monitor_month'])['time1'].mean().mean()
    ...
    if ttidata.empty:
        ...
        TTI = ttiData.groupby(['monitor_year'])['real_complaint_number'].sum().mean()
...
def cal(task_no,start,end,project_code,is_insert=True):
    ...
    mysql.clear_result_by_taskno(task_no=task_no
                                 , table_names=['tb_model_op_bridge_result_esi', 'tb_model_op_bridge_result_ssi',
                                                'tb_model_op_bridge_result_tsi','tb_model_op_bridge_result_usi',
                                                'tb_model_op_bridge_result_mid_evaluation','tb_model_op_bridge_result_all_evaluation'])
    print("---------------删除数据成功--------------")
