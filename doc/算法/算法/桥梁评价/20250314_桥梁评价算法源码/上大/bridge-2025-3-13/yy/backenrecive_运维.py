...
def msg4(body,msgdate):
    ...
    error=None
    try:
        calculate_运营.cal(body['TaskNO'],body['DataSource_StartDate'],body['DataSource_EndDate'],body['ProjectID'])
        #20240223 更新状态
...
exchange='MQ_Bridge_OSEVA'
queue='MQ_Bridge_OSEVA'
#本地
...
def callback(channel,method,properity,body):
    ...
    try:
        if body_json['Facility_Type'] == 'Bridge':
            if body_json['Model_Type'] == 'OSEVA':
                conn = mysql.setconn()
                re = pd.read_sql('select * from tb_model_op_bridge_result_all_evaluation', conn)
                ...
                print('%s模型不属于运营评估模型' % body_json['Model_Type'])
                status = 'Disable'
                daytime = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
                message = {'TaskNO': body_json['TaskNO'], 'ModelStatus': status, 'GenDate': daytime}
                send.send(message, exchange, 'web', 'web')
