...
def cal(task_no,project_code,is_insert=True):
    ...
    mysql.clear_result_by_taskno(task_no=task_no
                                 , table_names=['tb_model_af_result_category', 'tb_model_af_result_type',
                                                'tb_model_af_result'])
    print("---------------删除数据成功--------------")

    #执行函数计算单个子设施
    Tool_FS2.start_cal_single_fs2_task(task_no,project_code)
    ...
    #仅用projectCode读取
    facilitylist = pd.read_sql("select * from tb_model_bridge_af_facilitylist_2 where project_code='" + project_code + "'  ", conn)
    print(facilitylist.shape)
    checkvalue = pd.read_sql(
        "select * from tb_model_bridge_af_facilitycheck_value_2 where project_code='" + project_code + "' and task_no='" + task_no + "' ",
        conn)
    ...
    weight = pd.read_sql("select * from tb_model_bridge_af_type_weight_2 where facility_type='Bridge' ", conn)
