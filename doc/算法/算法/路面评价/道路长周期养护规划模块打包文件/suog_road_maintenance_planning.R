...
t1 <- Sys.time()
#读取数据
static <- read_excel(path = 'D:/Works/道路数据挖掘/道路静态数据-运营集团.xlsx',col_types = c(rep("text",2),rep("numeric",2),rep("text",2),rep("numeric",2),rep("text",3),rep("numeric",3),rep("text",2),rep(c("text","numeric"),7),"text","date","numeric"))
dynamic <- read_excel(path = 'D:/Works/道路数据挖掘/初始动态数据.xlsx',col_types = c(rep("text",2),rep("numeric",2),rep("text",2),rep("numeric",2),rep("text",3),rep("numeric",15),"date"))
history <- read_excel(path = 'D:/Works/道路长周期养护规划/外环北段案例/外环完整养护历史2011-2024.xlsx')
lane_traffic <- read_excel(path = 'D:/Works/道路长周期养护规划/外环北段案例/外环交通量数据-车道级.xlsx')
section_traffic <- read_excel(path = 'D:/Works/道路长周期养护规划/外环北段案例/外环交通量数据-断面级.xlsx')
##限定到外环北段
